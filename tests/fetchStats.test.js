...
beforeEach(() => {
    ...
    mock.onPost("https://api.github.com/graphql").reply((cfg) => {
        return [
            200,
            cfg.data.includes("contributionsCollection") ?
                data_stats
            :   data_repo,
        ];
    });
...
describe("Test fetchStats", () => {
    it("should fetch correct stats", async () => {
        let stats = await fetchStats("gzirysz");
    ...
    it("should throw error", async () => {
        ...
        await expect(fetchStats("gzirysz")).rejects.toThrow(
    ...
    it("should fetch total commits", async () => {
        mock.onGet(
            "https://api.github.com/search/commits?q=author:gzirysz",
        ).reply(200, { total_count: 1000 });

        let stats = await fetchStats("gzirysz", true);
        const rank = calculateRank({
            all_commits: true,
            commits: 1000,
            prs: 300,
    ...
    it("should throw specific error when include_all_commits true and API returns error", async () => {
        ...
        expect(fetchStats("gzirysz", true)).rejects.toThrow(
    ...
    it("should exclude stars of the `test-repo-1` repository", async () => {
        ...
        let stats = await fetchStats("gzirysz", true, ["test-repo-1"]);
        const rank = calculateRank({
            all_commits: true,
            commits: 1000,
            prs: 300,
    ...
    it("should fetch additional stats when it requested", async () => {
        let stats = await fetchStats("gzirysz", false, [], true, true, true);
        const rank = calculateRank({
            all_commits: false,
            commits: 100,
            prs: 300,
