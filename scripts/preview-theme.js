...
export const run = async () => {
            ...
            if (colors) {
                ...
                if (missingKeys.length > 0 || extraKeys.length > 0) {
                    for (const missingKey of missingKeys) {
                        errors.push(
                            `Theme color properties \`${missingKey}\` are missing`,
                        );
                    }
    ...
    `;
        ...
        const reviewReason =
            themesValid ? undefined : INVALID_REVIEW_COMMENT(comment_url);
        if (DRY_RUN) {
        ...
        debug(
            "Set review state to `REQUEST_CHANGES` and add `invalid` label...",
        );
        if (DRY_RUN) {
