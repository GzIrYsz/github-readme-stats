...
const renderStatsCard = (stats, options = {}) => {
    ...
    const apostrophe =
        ["x", "s"].includes(name.slice(-1).toLocaleLowerCase()) ? "" : "s";
    const i18n = new I18n({
        locale,
    ...
    const calculateTextWidth = () => {
        return measureText(
            ...
            : statItems.length ? i18n.t("statcard.title")
  ...
  */
    ...
    const defaultCardWidth =
        (hide_rank ? CARD_DEFAULT_WIDTH
        : statItems.length ? RANK_CARD_DEFAULT_WIDTH
        : RANK_ONLY_CARD_DEFAULT_WIDTH) + iconWidth;
    let width =
        card_width ?
            isNaN(card_width) ? defaultCardWidth
            :   card_width
        :   defaultCardWidth;
